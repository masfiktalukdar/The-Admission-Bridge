...
export const ApplyModal = ({ isOpen, onClose, university, userStats, userId, onSuccess }: ApplyModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userStats.gpa < university.gpa_required || userStats.ielts < university.ielts_required) {
        throw new Error("Application Rejected: Your academic scores do not meet the minimum requirements.");
      }

      if (!userId) {
         throw new Error("Authentication Error: You must be signed in to apply.");
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      ...
      >
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 p-8 text-white">
          ...
          <p className="text-indigo-200 text-sm mt-3 font-medium">Step {step} of 2</p>
        </div>
          ...
          {step === 1 ? (
            ...
            <div className="space-y-6">
               <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
                <h4 className="text-sm font-bold text-gray-900 mb-3 uppercase tracking-wider">Academic Verification</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
                    <span className="block text-gray-500 text-xs uppercase font-semibold">Your GPA</span>
                    <span className={`font-mono text-lg font-bold ${userStats.gpa < university.gpa_required ? 'text-red-600' : 'text-green-600'}`}>
                      {userStats.gpa}
                    </span>
                    <span className="text-xs text-gray-400 block mt-1">Req: {university.gpa_required}</span>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
                    <span className="block text-gray-500 text-xs uppercase font-semibold">Your IELTS</span>
                    <span className={`font-mono text-lg font-bold ${userStats.ielts < university.ielts_required ? 'text-red-600' : 'text-green-600'}`}>
                      {userStats.ielts}
                    </span>
                    <span className="text-xs text-gray-400 block mt-1">Req: {university.ielts_required}</span>
                  </div>
                </div>
              </div>
              
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1.5">Statement of Purpose</label>
                <textarea 
                  required
                  rows={4}
